...
class Time_series:  # on simule un tableau de mesures
    def __init__(
        ...
        offset : float,
        depth_sensors: list,
        param_time_dates: list = [None,None,DEFAULT_time_step], 
        param_dH_signal: list = DEFAULT_dH,
        param_T_riv_signal: list = DEFAULT_T_riv,  
        param_T_aq_signal: list = DEFAULT_T_aq,  
        sigma_meas_P: float = None,  
        sigma_meas_T: float = None
    ):
    # liste [date début, date fin, pas de temps (constant)], format des dates tuple datetime ou none
    # liste [amplitude (m), période (s), offset (m)] pour un variation sinusoïdale
    # liste [amplitude (°C), période (en seconde), offset (°C)] pour un variation sinusoïdale
    # liste [amplitude (°C), période (en seconde), offset (°C)] pour un variation sinusoïdale
    # (m) écart type de l'incertitude sur les valeurs de pression capteur
    # (°C) écart type de l'incertitude sur les valeurs de température capteur
